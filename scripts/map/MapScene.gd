...
const NODE_ICONS = {"battle":"⚔","event":"📜","shop":"🏮","rest":"🕯","boss":"☠"}
...
func _render_map() -> void:
	...
	for layer_idx in len(map_data):
		...
		for nd in map_data[layer_idx]:
			hbox.add_child(_make_btn(nd, is_current))
...
func _make_btn(nd: Dictionary, is_current: bool) -> Button:
	var btn   = Button.new()
	...
	var cap = nd
	btn.pressed.connect(func(): _on_node(cap))
	return btn

func _cn(t: String) -> String:
	match t:
		"battle": return "战斗"
		"event":  return "事件"
		"shop":   return "商店"
...
func _on_node(nd: Dictionary) -> void:
	nd["visited"] = true
	GameState.advance_node(nd["id"])
	var ntype = nd.get("type", "battle")

	if ntype == "rest":
		GameState.heal(int(GameState.max_hp * 0.30))
		_check_layer_done()
		_render_map()
		return
...
func _random_enemy() -> String:
	...
	var pool = json.get_data().get("enemies", []).filter(
		func(e): return e.get("layer",1) == GameState.current_layer and e.get("type","") == "normal"
	)
...
func _check_layer_done() -> void:
	...
	if map_data[idx].all(func(n): return n.get("visited", false)):

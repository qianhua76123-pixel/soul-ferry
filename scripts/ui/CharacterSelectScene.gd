...
func _build_ui() -> void:
	...
	title.offset_top = 40; title.offset_bottom = 90
	add_child(title)
	...
	subtitle.offset_top = 92; subtitle.offset_bottom = 120
	add_child(subtitle)
	...
	row.offset_left = -540; row.offset_right  = 540
	row.offset_top  = -180; row.offset_bottom = 220
	add_child(row)
	...
	confirm_btn.offset_top = -100; confirm_btn.offset_bottom = -48
	confirm_btn.offset_left = -110; confirm_btn.offset_right = 110
...
func _build_character_card(index: int) -> Control:
	...
	# 用 PlayerPixelArt 生成对应角色立绘
	portrait.texture = PlayerPixelArt.create_texture("idle", char_id)

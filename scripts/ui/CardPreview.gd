...
func _build() -> void:
	...
	style.bg_color        = Color(0.07, 0.04, 0.02, 0.96)
	style.border_color    = Color(0.80, 0.65, 0.15)
	...
	art.color = Color(0.12, 0.08, 0.06)
	vbox.add_child(art)
	...
	_title_lbl.add_theme_color_override("font_color", Color(0.95, 0.88, 0.70))
	...
	_desc_lbl.add_theme_color_override("font_color", Color(0.80, 0.76, 0.68))
...
func show_preview(card: Dictionary, anchor_pos: Vector2) -> void:
	...
	_cost_lbl.text    = "● %d" % int(card.get("cost", 1))
	_desc_lbl.text    = card.get("desc", "")
	...
	_rarity_lbl.add_theme_color_override("font_color",
		{"common": Color(0.7,0.7,0.7),
		 ...
		 "legendary": Color(1.0,0.8,0.1)}.get(rarity, Color.GRAY))
